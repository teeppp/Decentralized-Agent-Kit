...
| 4 | LiteLLM は Anthropic 以外のモデルには思考を送り返さないので、**モデルのプロンプトには思考が入らない**が、要約プロンプトには全文入る。だから要約リクエストのほうが 2.5 倍大きい | `lite_llm.py` / `contents.py` |
...
2. **予算への当てはめ**: 描画した履歴が `DAK_COMPACTION_INPUT_RATIO`（既定 0.5）× 窓を
   ...
   それでも超えるなら **古い嵩張るエントリから落とす**。先頭の密なエントリ（元の依頼か
   前回の要約）と最新のエントリは最後まで残す。
3. **モデルに拒否された場合**: 窓超過のエラーなら予算を半分にして再試行（3 回まで）。
   それでも拒否されたら、当てはめ済みの抜粋そのものを要約として圧縮イベントにする。
...
5. 要約プロンプトに「数百語に収める」を追加（6.8 tok/s の環境で 3,500 トークンの要約を
   9 分かけて書いていた）。
6. `_part_tokens`（リクエストガードの見積もり）で、署名のない思考パートを数えないようにした。
   LiteLLM はそれをモデルに送らないため、数えると llama.cpp 構成で予算を過剰に消費して
   ツール応答を必要以上に消していた。
