...
- 指示に入れる計画は、窓の 5%（1,000〜8,000 文字、`HarnessSettings.plan_chars`）までにする（#364）。超えたら、まず done の項目を件数の 1 行にまとめ、それでも超えたら項目の区切りで切って `read_plan` を案内する。`read_plan` と state の計画は切り詰めない。
