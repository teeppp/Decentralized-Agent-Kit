...
# Own Compose project: the shared helper removes volumes after a run (see
# scripts/smoke_common.sh), so the default project would delete the
# application's Postgres data.
